...
The prototype is built with strict TypeScript, Next.js, React, and Zod. It supports Agent Receipt Native Trace v1, one documented OTLP/JSON GenAI shape, and unfamiliar JSON record arrays through a reviewer-confirmed mapping manifest retained with the receipt. Packet assembly and verification are deterministic. The browser receives no model credentials, Granite cannot change the verdict, and missing evidence stays unknown.
...
- A vendor-shaped ten-record generic JSON log maps through explicit field paths and value translations to the same qualified clean authority result as its equivalent native example; unmapped values remain material-unparsed.
...
These are synthetic-prototype results, not claims about universal policy coverage, production scale, legal compliance, trace completeness, or real-world false-positive rates. Treat the linked demo as release evidence only after its exact deployed SHA and post-deployment checks are recorded in `docs/RELEASE_QA.md`.
